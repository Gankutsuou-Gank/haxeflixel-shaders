...
//made by yoshi engine  
//modified by Jonnycat
class ShadowEffect {
  public var shader:ShadowShader = new ShadowShader();
  public function new(){
    shader.diffX.value = [0]; 
    shader.diffY.value = [0]; 
  } 
  public function setX(yyyyy:Float){
    shader.diffX.value = [yyyyy];
  } 
  public function setY(xxxxx:Float){
    shader.diffY.value = [xxxxx];
  } 
}
